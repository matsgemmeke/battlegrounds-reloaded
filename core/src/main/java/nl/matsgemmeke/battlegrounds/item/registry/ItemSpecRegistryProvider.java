...
public class ItemSpecRegistryProvider implements Provider<ItemSpecRegistry> {
    ...
    @Inject
    public ItemSpecRegistryProvider(ObjectValidator objectValidator, SpecDeserializer specDeserializer, @Named("ItemsFolder") File itemsFolder, @Named("Battlegrounds") Logger logger) {
    ...
    @Override
    public ItemSpecRegistry get() {
        if (!itemsFolder.exists()) {
            itemsFolder.mkdirs();
            this.copyResourcesFiles(itemsFolder);
        }

        ItemSpecRegistry registry = new ItemSpecRegistry();
        ...
        for (File itemFolderFile : itemFolderFiles) {
            if (itemFolderFile.isDirectory()) {
                ...
                for (File itemSubfolderFile : itemSubfolderFiles) {
                    this.readItemFile(itemSubfolderFile, registry);
                }
            } else {
                this.readItemFile(itemFolderFile, registry);
            }
        }
    ...
    private void readItemFile(File itemFile, ItemSpecRegistry registry) {
        String id = null;
    ...
    private void addItemSpec(ItemSpecRegistry registry, File file, YamlConfiguration configuration) {
        ...
        if (configuration.getString("equipment-type") != null) {
            ...
            registry.addEquipmentSpec(name, equipmentSpec);
            return;
        }
        ...
        if (configuration.getString("gun-type") != null) {
            ...
            registry.addGunSpec(name, gunSpec);
            return;
        }
        ...
        if (file.getParentFile().getName().endsWith("melee_weapons")) {
            ...
            registry.addMeleeWeaponSpec(name, meleeWeaponSpec);
            return;
        }
