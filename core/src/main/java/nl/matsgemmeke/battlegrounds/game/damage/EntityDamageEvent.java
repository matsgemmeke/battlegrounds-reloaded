 ...
 *
 * @param victim  the entity being attack
 * @param damager the attacking entity
 * @param damage  the caused damage
 */
public record EntityDamageEvent(GameEntity victim, GameEntity damager, DamageNew damage) {

    public EntityDamageEvent modifyDamageAmount(double damageAmount) {
        return new EntityDamageEvent(victim, damager, new DamageNew(damageAmount, damage.type()));
    }
}
